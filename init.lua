...
--
--
-- a copy of the iqm.load() function from iqm-exm/ without love.graphics.newMesh
-- the necessary information to create the mesh is stored in objects.mesh_data, namely
--
-- objects.mesh_data.layout
-- objects.mesh_data.blob
-- objects.mesh_data.indices
--
-- these can be used to create a mesh as follows
-- local m = love.graphics.newMesh(layout, blob, "triangles")
-- m:setVertexMap(indices)
--
-- after mesh creation mesh_data can be set to nil and garbage collected
--
function iqm.load_threadsafe(file, save_data, preserve_cw)
	-- HACK: Workaround for a bug in LuaJIT's GC - we need to turn it off for the
	-- rest of the function or we'll get a segfault shortly into these loops.
	--
	-- I've got no idea why the GC thinks it can pull the rug out from under us,
...
-- exact same as iqm.load_anims but
-- instead of cpml vectors&quaternions it just returns tables
-- so it can be sent using love.thread
function iqm.load_anims_threadsafe(file)
	-- Require CPML here because loading the mesh does not depend on it.
	local cpml = require "cpml"

	local function vec3(a,b,c) return {a,b,c} end 
	local function quat(a,b,c,d) return {a,b,c,d} end 
